...
describe('rabbit service connect ', () => {
  ...
  const connection = {
    config: {
      user: 'testuser', password: '1q2w3e4r5t', host: 'somehost', port: '27017',
    },
  };
  const connectionWrong = {
    config: {
      user: 'testuser', password: '123', host: 'somehost', port: '27017',
    },
  };
  ...
  beforeEach(async () => {
    rabbitConnectStub.withArgs(dburlOK).returns(new Promise((resolve) => {
      resolve(connResult);
    }));
    ...
    rabbit.connect(req, res, session, connection)
      .then((resultConnection) => {
        ...
        assert.deepEqual(resultConnection, {
          connected: true, user: 'testuser', host: 'somehost', port: '27017',
        });
        done();
      });
  });
